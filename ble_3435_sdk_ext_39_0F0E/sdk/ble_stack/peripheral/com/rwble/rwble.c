    ...
    while (1)
    {
        ...
        if (irq_stat & BLE_SLPINTSTAT_BIT)
        {
            ...
            if(rc_sleep_flag)
			{
                ...
                uint32_t base_correct_number ;
                uint32_t fine_correct_number ;
				rc_sleep_flag = 0;
				BLE_sleep_time=(unsigned int)(31.25*(REG_AHB10_RW_DEEPSLDUR)+0.5);
				base_correct_number = BLE_sleep_time/625;
				fine_correct_number = 624 - BLE_sleep_time%625;
				REG_AHB10_RW_BASETIMECNTCORR = base_correct_number;
				REG_AHB10_RW_FINECNTCORR = fine_correct_number;
				REG_AHB10_RW_DEEPSLCNTL = 0x8|(REG_AHB10_RW_DEEPSLCNTL);
			}
			else
			{
				// Handle wake-up
